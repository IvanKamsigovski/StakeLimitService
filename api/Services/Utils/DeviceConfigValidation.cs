...
namespace api.Services.Utils
{
    public static class DeviceValidations
    {
        ...
        public static async Task<bool> IsDeviceBlockedAsync(IDeviceRepository deviceRepository, Device device, DateTime currentTime)
        {
            if (!device.IsDeviceBlocked)
                return false;

            if (device.BlockedAt == null)
            {
                device.IsDeviceBlocked = false;
                await deviceRepository.SaveDeviceChangesAsync();
        ...
        /// <summary>
        /// Checks if the stake exceeds the limit for the device.
        /// /// </summary>
        /// /// <returns>True if the stake exceeds the limit, false otherwise.</returns>
        public static async Task<bool> IsStakeOverTheLimit(IDeviceRepository deviceRepository, Device device, double totalStakeSum)
        {
